...
func TestHandleParseError(t *testing.T) {
	...
	responseReader := buffer.NewReader(slogt.New(t), outBuf, buffer.DefaultBufferSize)

	// 1. Expect BindComplete
	msgType, _, err := responseReader.ReadTypedMsg()
	require.NoError(t, err)
	assert.Equal(t, types.ClientMessage(types.ServerBindComplete), msgType)

	// 2. Expect ErrorResponse
	msgType, _, err = responseReader.ReadTypedMsg()
	require.NoError(t, err)
	assert.Equal(t, byte(types.ServerErrorResponse), byte(msgType))
}
