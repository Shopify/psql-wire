...
// TestShutdownWithoutConnections tests shutdown with no connections
func TestShutdownWithoutConnections(t *testing.T) {
	handler := func(ctx context.Context, query string) (PreparedStatements, error) {
		statement := NewStatement(func(ctx context.Context, writer DataWriter, parameters []Parameter) error {
	...
	// Shutdown immediately with no connections
	start := time.Now()
	err = server.Close()
	duration := time.Since(start)
	...
	if duration > 100*time.Millisecond {
		t.Errorf("Expected very fast shutdown with no connections, but took %v", duration)
	}

	t.Logf("Shutdown with no connections completed in %v", duration)
}